...
#ifdef ENABLE_HDF
int mat2hdf5(double *wdata, int timeCount, int particleCount, const char *FILE,
             const char *DATASET) {
...
int main(int argc, char **argv) {
    // Every nuclei for every time step. Does it exist (1), or has it decayed
    // (0). This would make sense to be an int, but we'll use float64 here as
    // you'll probably want to deal with that datatype more often in your
    // projects etc.
    ...
    for (int t = 1; t < T; t++) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (random_float < DECAY_CHANCE) { // Now I am become Death. Decay,
